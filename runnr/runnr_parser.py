...
class runnr_parser():
    ...
    def runnr_parse_line(self, line : str, line_no : int):
        ...
        if line.find('::') == -1:
            print(f'runnr: error: config: no argument separater \'::\' found in >> {line} >> at line no. {line_no}')
            exit(1)
    ...
    def runnr_parse_extension(self, un_p_extension : str, line_no : int, parsed_data : dict) -> None:
        extension = un_p_extension[un_p_extension.find('(') + 1 : un_p_extension.find(')')].replace(' ', '')

        if not extension[1:].isalpha():
            print(f'runnr: error: config: {extension} is not a valid extension at line no. {line_no}')
            exit(1)
    ...
    def runnr_parse_tokens(self, un_p_arguments : str, line_no : int, parsed_data : dict) -> None:
        ...
        for i in range(len(tokens)):
            ...
            if tokens[i].find('"') == -1:
                print('runnr: syntax error: config: all arguments\'s parameters much be wrapped in ""')
                exit(1)
    ...
    def runnr_set_args_param(self, args : str, param : str, line_no : int, parsed_data : dict):
        match args:
            ...
            case _:
                print(f'runnr: error: config: unknown argument >> {args} >> at line no. {line_no}')
                exit(1)
